...
class TwoPyroxeneThermometer(Thermometer):
	def __init__(self,opx, cpx, pressure=1.5):
		self.formula = {
			"opx": opx.get_cations(6),
			"cpx": cpx.get_cations(6)
		}
		self.P = pressure #GPa

class Ca_OPX(Thermometer):
	name = "Ca in OPX"
	def __init__(self, opx, cpx=None, pressure=1.5):
		self.opx = opx.get_cations(6)
		self.P = pressure #GPa
